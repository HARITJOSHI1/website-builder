...
const page = async ({ params }: Props) => {
  ...
  const prices = await stripe.prices.list({
    product: process.env.NEXT_COMFORT_PRODUCT_ID,
    active: true,
  });

  const subs = await stripe.subscriptions.list();
  const currentPlanActiveFromStripe = subs.data[0].items.data[0].plan;

  const currentPlanDetails = pricingCards.find(
    (c) => c.priceId === currentPlanActiveFromStripe.id
  );
  ...
  return (
    <>
      <h1 className="text-4xl p-4">Billing Details</h1>
      <Separator className="mb-6" />
      <h2 className="text-2xl p-4">Curent plan</h2>
      <div className="flex flex-col lg:!flex-row justify-between gap-8">
        <PricingCard
          ...
          further question contact support@comfort.com"
